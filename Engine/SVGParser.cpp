...
bool SVGParser::GetVerticesFromSvgFile(const std::string& filePath, std::vector<std::vector<Point2f>>& vertices)
{
    ...
    if (!svgStream)
    {
        std::cerr << "SVGParser::GetVerticesFromSvgFile(..), failed to load vertices from file " << filePath <<
            std::endl;
        return false;
    }
    ...
    {
        std::cerr << "SVGParser::GetVerticesFromSvgFile(..), malformed or unsupported information in file " << filePath
            << std::endl;
        return false;
    }
...
bool SVGParser::GetVerticesFromSvgString(std::string& svgString, std::vector<std::vector<Point2f>>& vertices)
{
    ...
    while (GetElementContent(svgString, "path", pathElementContent, startPosContent, endPosContent))
    {
        ...
        {
            std::cerr << "Empty verticesVector in GetVerticesFromSvgString(..), no vertices found in the path element"
                << std::endl;
            vertices.clear();
            return false;
        }
...
bool SVGParser::GetVerticesFromPathData(const std::string& pathData, std::vector<Point2f>& vertices)
{
    ...
    Point2f cursor{};
    Point2f startPoint{};
    //At the end of the z command, the new current point is set to the initial point of the current subpath.
...
bool SVGParser::GetElementContent(const std::string& svgText, const std::string& elementName,
                                  std::string& elementContent, size_t& startContentPos, size_t& endContentPos)
{
    // 2 possible formats
...
bool SVGParser::GetAttributeValue(const std::string& svgText, const std::string& attributeName,
